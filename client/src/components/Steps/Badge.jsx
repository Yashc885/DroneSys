...
// Function to determine color classes based on the color prop
const getColorClass = (color, hasHover = true) => {
  switch (color) {
    case "pink":
...
const Badge = ({
  ...
  return !!href ? (
    <Link
      href={href || ""}
      className={`transition-colors hover:text-white duration-300 ${CLASSES} ${getColorClass(color)}`}
    >
      {name}
    </Link>
  ) : (
    <span className={`${CLASSES} ${getColorClass(color, false)} ${className}`}>
      {name}
    </span>
  );
};
